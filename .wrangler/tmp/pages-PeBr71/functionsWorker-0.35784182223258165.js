...
app.post("/tickets", async (c) => {
  const { title, description, location, priority, type, client_id, photos } = await c.req.json();
  const stmt = c.env.DB.prepare(`
    INSERT INTO tickets (title, description, location, priority, type, client_id)
    VALUES (?, ?, ?, ?, ?, ?) RETURNING id
  `);
  const info = await stmt.bind(title, description, location, priority, type, client_id).first();
...
// ../.wrangler/tmp/pages-PeBr71/functionsRoutes-0.2406501895271056.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
