...
app.post("/tickets", async (c) => {
  const { title, description, location, priority, type, sector, client_id, photos } = await c.req.json();
  const stmt = c.env.DB.prepare(`
    INSERT INTO tickets (title, description, location, priority, type, sector, client_id)
    VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id
  `);
  const info = await stmt.bind(title, description, location, priority, type, sector, client_id).first();
...
// ../.wrangler/tmp/pages-pHpM6b/functionsRoutes-0.6382634308236224.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
