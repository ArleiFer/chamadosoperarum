...
app.post("/auth/sync", async (c) => {
  try {
    ...
    const isSuperAdmin = targetEmail === "ti@venancio.eco.br" || targetName.includes("arlei") || targetEmail.includes("arlei");
    if (isSuperAdmin) {
      role = "admin";
    }
...
app.put("/users/:id", async (c) => {
  ...
  const { name, role, company } = await c.req.json();
  await c.env.DB.prepare("UPDATE users SET name = ?, role = ?, company = ? WHERE id = ?").bind(name, role, company, id).run();
...
app.get("/service-types", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM service_types ORDER BY name").all();
  return c.json(results || []);
});
app.post("/service-types", async (c) => {
  const { name } = await c.req.json();
  const stmt = c.env.DB.prepare("INSERT INTO service_types (name) VALUES (?) RETURNING *");
  const info = await stmt.bind(name).first();
  return c.json(info);
});
app.put("/service-types/:id", async (c) => {
  const id = c.req.param("id");
  const { name } = await c.req.json();
  await c.env.DB.prepare("UPDATE service_types SET name = ? WHERE id = ?").bind(name, id).run();
  return c.json({ success: true });
});
...
app.get("/tickets/:id", async (c) => {
  ...
  return c.json({
    ...ticket,
    comments: comments || [],
    photos: photos ? photos.map((p) => p.photo_data) : []
  });
});
app.post("/tickets", async (c) => {
  const { title, description, location, priority, type, sector, company, client_id, photos } = await c.req.json();
  const stmt = c.env.DB.prepare(`
    INSERT INTO tickets (title, description, location, priority, type, sector, company, client_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING id
  `);
  const info = await stmt.bind(title, description, location, priority, type, sector, company, client_id).first();
  const ticketId = info?.id;
  if (ticketId && photos && Array.isArray(photos) && photos.length > 0) {
    const stmts = photos.map(
      (photo) => c.env.DB.prepare("INSERT INTO ticket_photos (ticket_id, photo_data) VALUES (?, ?)").bind(ticketId, photo)
    );
...
app.put("/tickets/:id/status", async (c) => {
  const id = c.req.param("id");
  const { status, provider_id } = await c.req.json();
...
app.get("/dashboard", async (c) => {
  const { results: byType } = await c.env.DB.prepare("SELECT type as name, COUNT(*) as value FROM tickets GROUP BY type").all();
  const { results: byStatus } = await c.env.DB.prepare("SELECT status as name, COUNT(*) as value FROM tickets GROUP BY status").all();
  const { results: byCompany } = await c.env.DB.prepare("SELECT IFNULL(company, 'N\xE3o Informada') as name, COUNT(*) as value FROM tickets GROUP BY company").all();
  const { results: slaData } = await c.env.DB.prepare(`
    SELECT type as name, AVG(julianday(updated_at) - julianday(created_at)) * 24 as avg_hours
    FROM tickets
...
// ../.wrangler/tmp/pages-lqAFxR/functionsRoutes-0.31890742336191014.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
