    ...
    if (ticketId) {
      ...
      if (allStaff) {
        for (const s of allStaff) {
          if (s.id !== client_id) {
            c.executionCtx.waitUntil(createNotification(db, s.id, "Novo Chamado", `Protocolo #${ticketId}: ${title}`, "info", ticketId));
          }
        }
      }
    }
...
// ../.wrangler/tmp/pages-U4QjKI/functionsRoutes-0.06849180943101862.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
