...
app.post("/users", async (c) => {
  ...
  const stmt = c.env.DB.prepare("INSERT INTO users (name, email, role) VALUES (?, ?, ?) RETURNING *");
  const info = await stmt.bind(name, email, role).first();
  return c.json(info);
});
app.put("/users/:id", async (c) => {
  const id = c.req.param("id");
  const { name, role } = await c.req.json();
  await c.env.DB.prepare("UPDATE users SET name = ?, role = ? WHERE id = ?").bind(name, role, id).run();
...
// ../.wrangler/tmp/pages-Gu2mOF/functionsRoutes-0.5912507379680635.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
