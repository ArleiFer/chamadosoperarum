  ...
  if (ticketId) {
    ...
    if (allStaff) {
      ...
      for (const s of allStaff) {
        if (s.id !== client_id) {
          c.executionCtx.waitUntil(createNotification(db, s.id, "Novo Chamado", `Protocolo #${formattedId}: ${title}`, 'info', formattedId as string));
        }
      }
    }
  }
...
app.put('/tickets/:id/materials', async (c) => {
  const id = c.req.param('id');
  const { materials } = await c.req.json();
  await c.env.DB.prepare("UPDATE tickets SET materials = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(materials, id).run();
  return c.json({ success: true });
});
...
app.get('/dashboard', async (c) => {
  ...
  // 10. Reabertura
  const reopens = await db.prepare(`SELECT COUNT(*) as count FROM tickets WHERE reopen_reason IS NOT NULL ${where}`).bind(...params).first() as any;

  // 11. Chamados Abertos
  const openTickets = await db.prepare(`SELECT COUNT(*) as count FROM tickets WHERE status IN ('Em Aberto', 'Aguardando Peças') ${where}`).bind(...params).first() as any;
  ...
  const avgNps = await db.prepare(`SELECT AVG(nps_score) as avg FROM tickets WHERE nps_score IS NOT NULL ${where}`).bind(...params).first() as any;
  ...
  const { results: byCompany } = await db.prepare(`SELECT IFNULL(company, 'Não Informada') as name, COUNT(*) as value FROM tickets ${clientId ? "WHERE client_id = ?" : ""} GROUP BY company`).bind(...params).all();

  // If clientId is provided, we should check if these stats are really for this client.
  // SQLite should handle this via the bind, but let's ensure params isn't empty if clientId exists.
